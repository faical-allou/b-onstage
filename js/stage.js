	...
	var methods = {
		...
		init : function(options){
			return this.each(function() {						
				...
				/**********INIT SEARCH FORM*********/
				$('#search-form-stage').submit(function() {
				...
				.click(function(event){
					...
					$.ajax({
						...
						success : function(data){
							$('#loader-more-stages').hide();
							$('#stages-list').append(data.text);	
							$('.button-show-profil').button();	
							$('.button-send-msg').button();	
							$('.button-add-contact').button();	
							if(data.show_more)
								$('#show-more-stages').show();							
						}						
					});				
				});
				...
				function show(){
					page = 1;
					$('#stages-list').animate({opacity:.2},200);
					$.ajax({
						url			: '/stages/show',
						type		: 'post',
						dataType	: 'json',
