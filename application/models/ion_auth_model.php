...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select('username, email, id, password, active, last_login, avatar')
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->hash_method = 'sha1' && $user->password === $password || $this->hash_method = 'bcrypt' && $password === true)
			{
				...
				$session_data = array(
					...
					'avatar'               => $user->avatar,
					'web_address'          => (isset($user->web_address)) ? $user->web_address : ''
				);
	 ...
	 * @author Ben Edmunds
	 **/
	public function user_by_username($name)
	{
		$this->trigger_events('user_by_username');

		$this->limit(1);
		$this->where($this->tables['users'].'.username', $name);
		$this->users();
		return $this;
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function user_by_web_address($name)
	{
		$this->trigger_events('user_by_web_address');

		$this->limit(1);
		$this->where($this->tables['users'].'.web_address', $name);
		$this->users();
		return $this;
	}
	...
	**/
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, username , email, avatar')
